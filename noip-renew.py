...
class Robot:
    ...
    def login(self) -> None:
        ...
        ele_usr = self.browser.find_element(by=By.NAME, value="username")
        ele_pwd = self.browser.find_element(by=By.NAME, value="password")
        ele_usr.send_keys(self.username)
        ele_pwd.send_keys(base64.b64decode(self.password).decode("utf-8"))
        self.browser.find_element(by=By.ID, value="clogs-captcha-button").click()
        if self.debug > 1:
            time.sleep(1)
            self.browser.save_screenshot("debug2.png")

    def update_hosts(self) -> bool:
        count = 0
    ...
    def open_hosts_page(self) -> None:
        time.sleep(5)
        self.browser.find_element(
            by=By.XPATH, value='//*[text()="in the past 30 days"]'
        ).click()
        ...
        try:
            ...
            WebDriverWait(self.browser, 90).until(element_present)
    ...
    def update_host(self, host_button, host_name):
        ...
        try:
            if (
                self.browser.find_elements(by=By.XPATH, value="//h2[@class='big']")[
                    0
                ].text
                == "Upgrade Now"
            ):
    ...
    def get_host_expiration_days(host, iteration):
        try:
            host_remaining_days = host.find_element(
                by=By.XPATH, value=".//a[@class='no-link-style']"
            ).text
        except:
        ...
        if regex_match is None:
            raise Exception(
                "Expiration days label does not match the expected pattern in iteration: {iteration}"
            )
    ...
    def get_host_link(host, iteration):
        return host.find_element(
            by=By.XPATH, value=".//a[@class='link-info cursor-pointer']"
        )
    ...
    def get_host_button(host, iteration):
        return host.find_element(
            by=By.XPATH,
            value=".//following-sibling::td[4]/button[contains(@class, 'btn')]",
        )

    def get_hosts(self):
        host_tds = self.browser.find_elements(
            by=By.XPATH, value='//td[@data-title="Host"]'
        )
        if len(host_tds) == 0:
